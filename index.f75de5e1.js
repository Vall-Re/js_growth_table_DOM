...
if (!table || !appendRowButton || !removeRowButton || !appendColButton || !removeColButton) throw new Error("Required DOM elements not found");
const MIN = 2;
const MAX = 10;
...
appendRowButton.addEventListener("click", ()=>{
    ...
    for(let i = 0; i < cellCount; i++)newRow.appendChild(document.createElement("td"));
...
removeRowButton.addEventListener("click", ()=>{
    ...
    if (trs.length) trs[trs.length - 1].remove();
    updateButtonsState();
});
appendColButton.addEventListener("click", ()=>{
    ...
    if (trs.length === 0) {
        for(let i = 0; i < MIN; i++){
            ...
            for(let j = 0; j < MIN; j++)tr.appendChild(document.createElement("td"));
